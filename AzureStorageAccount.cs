...
namespace AzureStorageAccountClassLib
{
    public class AzureStorageAccount : IAzureStorageAccount
    {
        ...
        public AzureStorageAccount(IConfiguration configuration, ILogger<AzureStorageAccount> logger)
        {
        ...
        public async Task<List<BlobDto>> ListAsync(string prefix = "")
        {
            ...
            await foreach (BlobItem file in container.GetBlobsAsync(prefix: string.IsNullOrWhiteSpace(prefix) ? null : prefix))
            {
                ...
                files.Add(new BlobDto
                {
                    Uri = fullUri,
                    Name = name,
                    CreateOn = file.Properties.CreatedOn?.ToString("yyyy-MM-dd HH:mm:ss") ?? "",
        ...
        public async Task<BlobResponseDto> UploadAsync(IFormFile blob, string path = "", string fileName = "")
        {
            ...
            try
            {
                fileName = string.IsNullOrWhiteSpace(fileName) ? blob.FileName : fileName;

                // Get a reference to the blob just uploaded from the API in a container from configuration settings
                BlobClient client = container.GetBlobClient(string.IsNullOrWhiteSpace(path) ? fileName : Path.Combine(path, fileName));
